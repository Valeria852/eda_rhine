...
runoff_stats <- runoff_day[, .(mean_day = round(mean(value), 0), sd_day = round(sd(value), 0), min_day = round(min(value), 0), max_day = round(max(value), 0)), by = sname]
head(runoff_stats, 4)
...
station_coords_sf <- st_as_sf(runoff_summary, coords = c("lon", "lat"), crs = 4326)
